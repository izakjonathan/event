          ...
          <div className="mt-4 flex gap-2 overflow-x-auto scrollbar-none">
            {STATUSES.map((status) => (
              <button
                ...
                className={`pill whitespace-nowrap border px-3 py-2 text-sm transition ${filter === status ? 'eos-primary' : 'eos-border eos-panel eos-muted'}`}
              >
                {status}
              </button>
            ))}
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(draft.links || {})
                  .filter(([, value]) => value)
                  .map(([key, value]) => (
                    <a key={key} href={String(value)} target="_blank" className="pill border eos-border eos-panel px-3 py-2 text-sm eos-muted">
                      {key}
                    </a>
                  ))}
              </div>
