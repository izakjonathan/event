  ...
  <AppShell title="Event Planner" actions={<Button kind="soft" onClick={save}>Save</Button>}>
   <div className="space-y-5">
    ...
    <Section title="Linked artists / lineup" right={`${draft.artists.length} artist${draft.artists.length === 1 ? '' : 's'}`}>
     {draft.artists.length === 0 && <p className="text-sm eos-muted">No artists linked yet. Add submissions from Artist Management.</p>}
    ...
    <Section title="Files">
     <label className="block cursor-pointer rounded-2xl border border-dashed eos-border eos-panel p-4 text-center text-sm eos-muted">
      ...
      {draft.files.map((file) => (
       <SubCard key={file.id}>
        <div>
         ...
         <p className="mt-1 text-xs eos-muted">{Math.round(file.size / 1024)} KB</p>
        </div>
